...
export default function EditException({ exception }: Props) {
  const { data, setData, put, processing, errors } = useForm({
    reason: exception.reason,
    description: exception.description,
    valid_from: exception.valid_from.split('T')[0],
    valid_until: exception.valid_until.split('T')[0],
  })

  const handleSubmit: FormEventHandler = (e) => {
    e.preventDefault()
    put(`/fcv/access-exceptions/${exception.id}`)
  }
  ...
  return (
    <AppLayout>
      <Head title="Editar Excepción - FCV" />
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">
              Editar Excepción de Acceso
            </h1>
            <p className="text-muted-foreground">
              Modificar excepción para {exception.person.name}
            </p>
          </div>
        </div>
