...
export default function AdminPermissionsIndex() {
    const { t } = useI18n()
    const page = usePage<{ props: PermissionsPageProps }>()
    const { permissions, filters } = page.props as unknown as PermissionsPageProps
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('Administración - Permisos')} />
            <div className="flex h-full flex-1 flex-col gap-4 overflow-x-auto rounded-xl p-4">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div className="text-lg font-medium">{t('Permisos')}</div>
                                ...
                                {permissions.data.length === 0 && (
                                    <tr>
                                        <td colSpan={3} className="px-4 py-6 text-center text-muted-foreground">
                                            {t('No hay permisos que coincidan con la búsqueda.')}
                                        </td>
                                    </tr>
                                )}
